...
router.get("/weekly", requireAuthMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayFoodLogs = userFoodLogs.filter((f) => f.date === dateStr);
      ...
      const caloriesConsumed = dayFoodLogs.reduce((sum, f) => sum + f.calories, 0);
...
router.get("/streak", requireAuthMiddleware, async (req: Request, res: Response) => {
  try {
    const userId = req.user.id;
    const userActivities = await db.select().from(activities).where(eq(activities.userId, userId));
    ...
    const uniqueDates = [...new Set(userActivities.map((a) => a.date))].sort(
